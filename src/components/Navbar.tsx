    ...
    <>
      <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${scrolled ? "bg-[var(--black)]/85 backdrop-blur-lg border-b border-white/[0.06]" : "border-b border-transparent"}`}>
        ...
        <nav className={`relative mx-auto flex max-w-[1280px] items-center justify-between px-8 lg:px-16 transition-all duration-500 ${scrolled ? "py-4" : "py-8"}`}>
          ...
          {/* Spacer for mobile toggle (keeps nav layout consistent) */}
          <div className="h-10 w-10 md:hidden" />
        </nav>
      </header>

      {/* Mobile toggle — fixed, above everything including the overlay */}
      <button
        className={`fixed right-8 z-[60] flex h-10 w-10 items-center justify-center text-[var(--white)] transition-all duration-500 md:hidden ${scrolled ? "top-4" : "top-8"}`}
      ...
      <div className={`fixed inset-0 z-[55] transition-all duration-300 md:hidden ${open ? "visible opacity-100" : "invisible opacity-0"}`}>
        <div className="absolute inset-0 bg-[var(--black)]" onClick={() => setOpen(false)} />
